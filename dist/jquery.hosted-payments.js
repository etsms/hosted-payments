/* ASP.NET SignalR JavaScript Library 2.4.2 (https://github.com/SignalR/SignalR/tree/main/src/Microsoft.AspNet.SignalR.JS) */
...
(function ($, window, undefined) {
  ...
  signalR.hub = $.hubConnection("/v3/adapters/transvault", { useDefaultPath: false });
...
(function ($, window, document, undefined) {
  ...
  hp.EntryType.KEYED_CARD_NOT_PRESENT = "KEYED_CARD_NOT_PRESENT"; // request type

  hp.RequestTypes = {};
  hp.RequestTypes.CREATE_INSTRUMENT = 0;
  hp.RequestTypes.CHARGE = 1;
  hp.RequestTypes.REFUND = 2;
  hp.RequestTypes.ERROR = 9; // entry type
  ...
  hp.Utils.payments = []; // expose inital boolean for embeded instrument
  hp.Utils.hasPaymentInstrument = false; // A variety of CSS based identifiers
  ...
  var replacePaymentPlan = function replacePaymentPlan(instrumentId, planId, callback) {
    setTimeout(function () {
      try {
        ...
        showLoader();
        makeRequest(requestPayload).then(
          function (replacePlanResponse) {
            ...
            callback(null, replacePlanResponse);
            hideLoader();
          },
  ...
  var log = (function log(undefined) {
    ...
    return function (params) {
      ...
      if (!hp.Utils.defaults.debug) return; // call handler extension which provides stack trace
  ...
  var createInstance = function createInstance($element, callback) {
    ...
    $element.html(
      $wrapper
        .replace("{{success}}", hp.Utils.plugins.Success.createTemplate().replace("{{redirectLabel}}", hp.Utils.defaults.defaultRedirectLabel).replace("{{successLabel}}", hp.Utils.defaults.defaultSuccessLabel))
    ...
    $types
      ...
      .on("click", function (e) {
        ...
        $types.removeClass(activeClass).eq(currentIndex).addClass(activeClass); // Wait for DOM to finish loading before querying
  ...
  var setCustomerInfo = function setCustomerInfo(name, phone, email, shouldEmailNotify, shouldSmsNotify) {
    if (shouldEmailNotify !== undefined) {
      hp.Utils.defaults.sendEmailNotifications = shouldEmailNotify;
    }

    if (shouldSmsNotify !== undefined) {
      hp.Utils.defaults.sendSmsNotifications = shouldSmsNotify;
    }

    if (name !== undefined && name !== "") {
      hp.Utils.defaults.customerName = name;
    }

    if (phone !== undefined && phone !== "") {
      hp.Utils.defaults.customerPhone = phone;
    }
  ...
  var getCustomerInfo = function getCustomerInfo() {
    return {
      sendEmailNotifications: hp.Utils.defaults.sendEmailNotifications,
      sendSmsNotifications: hp.Utils.defaults.sendSmsNotifications,
      customerName: hp.Utils.defaults.customerName,
      customerPhone: hp.Utils.defaults.customerPhone,
      customerEmail: hp.Utils.defaults.customerEmail,
    };
  };
  ...
  var buildSuccessResultObject = function buildSuccessResultObject() {
    return {
      ...
      context_id: hp.Utils.getContextId(),
    };
  };
        ...
        $.each(responses, function (index) {
          ...
          hp.Utils.makeRequest(statusRequest).then(function (statusResponse) {
            if (statusResponse.type === "Transaction" && typeof statusResponse.properties !== "undefined") {
              ...
              responses[currentCount].customer_signature = statusResponse.properties.signatureRef === null || statusResponse.properties.signatureRef === undefined || statusResponse.properties.signatureRef === "" ? "https://images.emoney.com/00000000" : statusResponse.properties.signatureRef;
  ...
  var retrieveTransactionStatus = function retrieveTransactionStatus(transactionId) {
    ...
    hp.Utils.makeRequest(statusRequest).then(function (statusResponse) {
      if (statusResponse.type === "Transaction" && typeof statusResponse.properties !== "undefined") {
        ...
        successResponse.customer_signature = statusResponse.properties.signatureRef === null || statusResponse.properties.signatureRef === undefined || statusResponse.properties.signatureRef === "" ? "https://images.emoney.com/00000000" : statusResponse.properties.signatureRef;
  ...
  var promptAvs = function promptAvs($element) {
    ...
    setTimeout(function () {
      ...
      var $stateOptions = Object.keys(states)
        .map(function (item) {
      ...
      var $submitAvs = $avsPrompt.find(".hp-avs-submit").attr("disabled", "disabled");
    ...
    $.ajax(requestObject)
      ...
      .fail(function (errorResponse) {
        if (isCaptchaRequiredException(errorResponse)) {
          ...
          injectCaptchaScript().then(
            function (tokenResponse) {
              setCaptchaVerificationToken(tokenResponse);
              log("Reruning previous request...");
              getObjectResponseFromData(data).captchaVerificationToken = getCaptchaVerificationToken();
  ...
  var checkHttpsConnection = function checkHttpsConnection() {
    if (!hasChecked && (location.hostname === "localhost" || location.hostname === "127.0.0.1" || location.hostname === "")) {
      hasChecked = true;
    }
  ...
  var clearCaptcha = function clearCaptcha() {
    var $captchaContainer = getInstance().$element.find(".hp-captcha-container");
    $captchaContainer.find("#hp-captcha-selector").empty();
    $captchaContainer.removeClass("hp-captcha-container-active");
  };

  var isCaptchaRendered = function isCaptchaRendered() {
    var $captchaContainer = getInstance().$element.find("#hp-captcha-selector");

    var result = $captchaContainer.html() !== "";

    if (result) {
      log("Captcha is already rendered on UI.");
      return true;
    }

    log("Captcha is NOT rendered on the UI");
...
(function ($, window, document, undefined) {
  ...
  CreditCard.prototype.init = function () {
    sessionId = hp.Utils.getSession().sessionToken; // utils call

    var context = hp.Utils.handleLegacyCssClassApplication("cc", this.$element),
      $parent = context.parent,
      $content = context.content; // Clean parent, notify on complete.
  ...
  CreditCard.prototype.createTemplate = function (defaultCardCharacters, defaultNameOnCardName, defaultDateCharacters) {
    ...
    var $html = [
      ...
      '<div class="hp-card-visual-number" id="cardnumber">' + defaultCardCharacters + "</div>",
      '<div class="hp-card-visual-name" id="fullname">' + defaultNameOnCardName + "</div>",
  ...
  CreditCard.prototype.handleCreditCardInput = function (cardNumber) {
    ...
    if ((cardType === null || cardType !== "emoney") && this.wasOnceEMoney) {
      ...
      $month
        .removeAttr("readonly")
        .val(date.getMonth() <= 9 ? "0" + date.getMonth() : date.getMonth())
        .trigger("change");
...
(function ($, window, document, undefined) {
  ...
  BankAccount.prototype.createTemplate = function (defaultName, defaultAccountNumberCharacters, defaultRoutingNumberCharacters, defaultBank) {
    ...
    var $accountType = ["Checking", "Savings"].map(function (value) {
  ...
  BankAccount.prototype.handleSubmit = function () {
    ...
    hp.Utils.makeRequest(createPaymentInstrumentRequest)
      .then(function (res) {
        if (res.isException) {
          $this.$parent.trigger("hp.submit", {
            type: 9,
...
(function ($, window, document, undefined) {
  ...
  var getMessage = function getMessage(event) {
    ...
    if (messageObject.message.startsWith("An error occured.")) {
      var extractedMessage = messageObject.message.replace("An error occured.", "").replace(/\n/, "").trim();
      ...
      if (extractedMessage.startsWith("{")) {
        ...
        if (messageAsJson.ModelState !== undefined) {
          ...
          Object.keys(messageAsJson.ModelState).forEach(function (element) {
...
/* jQuery.Payments (https://github.com/stripe-archive/jquery.payment) */
...
(function ($, window, document, undefined) {
  ...
  defaults.captchaCallback = function (promise) {
    promise.resolve();
  };
  defaults.terminalId = "";
  ...
  defaults.sendEmailNotifications = false;
  defaults.sendSmsNotifications = false;
  ...
  var initialize = function initialize() {
    ...
    if (typeof $element.data("promptForAvs") !== "undefined") {
      if ($element.data("promptForAvs").toString() === "false") {
        hp.Utils.defaults.promptForAvs = false;
      } else if ($element.data("promptForAvs").toString() === "true") {
        hp.Utils.defaults.promptForAvs = true;
      }
    }

    if (typeof $element.data("allowAvsSkip") !== "undefined") {
      if ($element.data("allowAvsSkip").toString() === "false") {
        hp.Utils.defaults.allowAvsSkip = false;
      } else if ($element.data("allowAvsSkip").toString() === "true") {
        hp.Utils.defaults.allowAvsSkip = true;
      }
    }

    if (typeof $element.data("allowHttpsSkip") !== "undefined") {
      if ($element.data("allowHttpsSkip").toString() === "false") {
        hp.Utils.defaults.allowHttpsSkip = false;
      } else if ($element.data("allowHttpsSkip").toString() === "true") {
        hp.Utils.defaults.allowHttpsSkip = true;
      }
    }
    ...
    if (typeof $element.data("customerPhone") !== "undefined") {
      hp.Utils.defaults.customerPhone = $element.data("customerPhone").toString();
    }

    if (typeof $element.data("customerEmail") !== "undefined") {
      hp.Utils.defaults.customerEmail = $element.data("customerEmail").toString();
    }

    if (typeof $element.data("sendEmailNotifications") !== "undefined") {
      if ($element.data("sendEmailNotifications").toString() === "false") {
        hp.Utils.defaults.sendEmailNotifications = false;
      } else if ($element.data("sendEmailNotifications").toString() === "true") {
        hp.Utils.defaults.sendEmailNotifications = true;
      }
    }

    if (typeof $element.data("sendSmsNotifications") !== "undefined") {
      if ($element.data("sendSmsNotifications").toString() === "false") {
        hp.Utils.defaults.sendSmsNotifications = false;
      } else if ($element.data("sendSmsNotifications").toString() === "true") {
        hp.Utils.defaults.sendSmsNotifications = true;
      }
    }
    ...
    if (typeof $element.data("defaultButtonLabel") !== "undefined") {
    ...
    if (typeof $element.data("paymentTypeOrder") !== "undefined") {
      hp.Utils.defaults.paymentTypeOrder = $.trim($element.data("paymentTypeOrder").toString().replace(" ", ""))
        .split(",")
        .map(function (item) {
          return +item;
        });
  ...
  $.extend(Plugin.prototype, {
    init: function init() {
      var $element = $(this.element); // Get outer wrapper width and set css class for mobile purposes
